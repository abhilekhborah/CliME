...
CHECKPOINT_FILE = 'gemini_flash_eval_checkpoint.pkl'
OUTPUT_FILE = 'gemini_flash_eval.json'
...
def get_gemini_response(prompt_text, max_retries=3):
    """
    Get a response from Gemini with retry logic for rate limiting
    """
    retries = 0
        ...
        for idx in tqdm(range(start_idx, len(dataset)), desc="Processing dataset"):
            ...
            try:
                ...
                for key, value in prompts.items():
                    ...
                    response = get_gemini_response(value)
